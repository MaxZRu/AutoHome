...
public:
    Modbus2();
    Modbus2(uint8_t u8id, uint8_t u8serno);
    Modbus2(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin);
    Modbus2(uint8_t u8id);
 ...
 * @ingroup setup
 * @overload Modbus2::Modbus(uint8_t u8id, uint8_t u8serno)
 * @overload Modbus2::Modbus(uint8_t u8id)
 * @overload Modbus2::Modbus()
 */
Modbus2::Modbus2(uint8_t u8id, uint8_t u8serno)
{
    init(u8id, u8serno, 0);
}
 ...
 * @ingroup setup
 * @overload Modbus2::Modbus(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
 * @overload Modbus2::Modbus(uint8_t u8id)
 * @overload Modbus2::Modbus()
 */
Modbus2::Modbus2(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
{
 ...
 * using "void Modbus2::begin(SoftwareSerial *softPort, long u32speed)".
 *
 * @param u8id   node address 0=master, 1..247=slave
 * @ingroup setup
 * @overload Modbus2::Modbus(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
 ...
 * @ingroup setup
 */
void Modbus2::begin(long u32speed)
{
 ...
 * @ingroup setup
 */
void Modbus2::begin(SoftwareSerial *sPort, long u32speed)
{
 ...
 * @ingroup setup
 */
void Modbus2::begin(long u32speed,uint8_t u8config)
{
 ...
 * Call once class has been instantiated, typically within setup().
 *
 * @overload Modbus2::begin(uint16_t u16BaudRate)
 * @ingroup setup
 */
void Modbus2::begin()
{
 ...
 * @ingroup setup
 */
void Modbus2::setID( uint8_t u8id)
{
    if (( u8id != 0) && (u8id <= 247))
    {
        this->u8id = u8id;
 ...
 * @ingroup setup
 */
void Modbus2::setTimeOut( uint16_t u16timeOut)
{
 ...
 * @ingroup loop
 */
boolean Modbus2::getTimeOutState()
{
 ...
 */
int8_t Modbus2::query( modbus_t telegram )
{
 ...
 * @ingroup loop
 */
int8_t Modbus2::poll( uint16_t *regs, uint8_t u8size )
{
...
void Modbus2::init(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
{
    this->u8id = u8id;
...
void Modbus2::init(uint8_t u8id)
{
    this->u8id = u8id;
    this->u8serno = 4;
    this->u8txenpin = 0;
 ...
 * @ingroup buffer
 */
uint16_t Modbus2::calcCRC(uint8_t u8length)
{
 ...
 * @ingroup buffer
 */
uint8_t Modbus2::validateRequest()
{
 ...
 * @ingroup buffer
 */
uint8_t Modbus2::validateAnswer()
{
    // check message crc vs calculated crc
    uint16_t u16MsgCRC =
        ((au8Buffer[u8BufferSize - 2] << 8)
 ...
 * @ingroup buffer
 */
void Modbus2::buildException( uint8_t u8exception )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus2::process_FC1( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup register
 */
int8_t Modbus2::process_FC3( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus2::process_FC5( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup register
 */
int8_t Modbus2::process_FC6( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus2::process_FC15( uint16_t *regs, uint8_t u8size )
{
 ...
 * @ingroup register
 */
int8_t Modbus2::process_FC16( uint16_t *regs, uint8_t u8size )
{
    uint8_t u8func = au8Buffer[ FUNC ];  // get the original FUNC code
